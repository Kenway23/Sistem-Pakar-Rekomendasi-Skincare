// FUNGSI COMBINE CERTAINTY FACTOR (Persis seperti Python)
        function combineCF(cf1, cf2) {
            return cf1 + cf2 * (1 - cf1);
        }

        function analisisSkincare() {
            // Ambil nilai checkbox (Persis seperti Python)
            const g01 = document.getElementById("g01").checked ? 1 : 0;  // Kulit Berminyak
            const g02 = document.getElementById("g02").checked ? 1 : 0;  // Kulit Kering
            const g03 = document.getElementById("g03").checked ? 1 : 0;  // Banyak Jerawat
            const g04 = document.getElementById("g04").checked ? 1 : 0;  // Kulit Kusam
            const g05 = document.getElementById("g05").checked ? 1 : 0;  // Kulit Sensitif
            const g06 = document.getElementById("g06").checked ? 1 : 0;  // Pori-pori Besar
            ...
            // RULE 1: IF Kulit Berminyak AND Banyak Jerawat AND Pori-pori Besar
            // THEN Oily Acne Prone
            if (g01 && g03 && g06) {
                let cf1 = 0.9 * g01;
                let cf2 = 1.0 * g03;
                let cf3 = 0.8 * g06;
                ...
                hasil["Oily Acne Prone"] = {
                    cf: r01,
                    saran: "Gunakan facial wash gentle, moisturizer gel, sunscreen non-comedogenic, dan produk dengan kandungan salicylic acid. Hindari skincare yang terlalu berminyak dan jangan terlalu sering memakai scrub."
                };
            }

            // RULE 2: IF Kulit Kering AND Kulit Kusam
            // THEN Dry & Brightening
            if (g02 && g04) {
                let cf1 = 1.0 * g02;
                let cf2 = 0.7 * g04;
            ...
            // RULE 3: IF Kulit Sensitif AND Banyak Jerawat
            // THEN Sensitive Skin Care
            if (g05 && g03) {
                let cf1 = 1.0 * g05;
                let cf2 = 0.7 * g03;
                ...
                hasil["Sensitive Skin Care"] = {
                    cf: r03,
                    saran: "Gunakan skincare yang gentle, fragrance-free, dan alcohol-free. Hindari scrub kasar, alkohol tinggi, parfum, atau bahan aktif yang terlalu kuat."
                };
            }

            // RULE 4: IF Kulit Berminyak AND Pori-pori Besar
            // THEN Normal to Oily / Pore Care
            if (g01 && g06) {
                let cf1 = 0.8 * g01;
                let cf2 = 0.9 * g06;
            ...
            // Menentukan hasil terbaik berdasarkan CF tertinggi
